...
class PatientOutcomeModel(nn.Module):
    ...
    def forward(self, flat_data, graph_data, patient_ids, ts_data):
        ...
        return risk_scores # risk_scores shape = (batch_size, time_steps)
...
#         # === get the entire graph embeddings in cpu ===
#         graph_x = graph_data.x.to(device)  # (N_nodes, D_graph)
#         edge_index = graph_data.edge_index.to(device)  # (2, N_edges)
#         graph_patient_ids = graph_data.patient_ids  # (N_nodes,)

#         # === ensure that the input data is in the same device ===
#         patient_ids = patient_ids.to("device")
#         graph_patient_ids = graph_patient_ids.to("device")

#         # === find the indices of the patients in the batch in the graph data ===
#         batch_indices = torch.tensor([
#             torch.where(graph_patient_ids == pid)[0][0] for pid in patient_ids
#         ], dtype=torch.long)  # (batch_size,)

#         # === extract the embeddings of the patients in the batch, and move to gpu ===
#         batch_graph_x = graph_x[batch_indices].to(device)

#         # === select only the edges that are in the batch ===
...
#         # === compute the graoh embeddings that are in the batch ===
#         batch_graph_embeddings = self.graph_encoder(batch_graph_x, batch_edge_index)  # (batch_size, D_graph)

#         # === calculate Flat Embeddings ===
#         flat_emb = self.flat_encoder(flat_data)  # (batch_size, D_flat)
...
#         return risk_scores  # (batch_size, time_steps)
