...
class MultiModalDataset(Dataset):
    def __init__(self, data_path,global_node2idx):
        ...
        self.ts_h5_file = os.path.join(self.data_path, 'timeseries_30_each_patient.h5')
        self.risks_h5_file = os.path.join(self.data_path, 'risks_30_each_patient.h5')
...
class VitalSignsDatasetKMeans(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return  aligned_ts_data, x_trans_mat, y_trans_mat, idx ,cat
...
def vital_dmm_collate_fn(batch):
    ...
    # 2. 解包数据
    ts_list, x_trans_list, y_trans_list, idx_list,cat_list = zip(*batch)
    ...
    cat_list     = [cat_list[i] for i in sorted_idx]

    # 5. 对每个序列进行填充 (padding)
    x_seq = pad_sequence(ts_list, batch_first=True, padding_value=0.0)
    x_trans_mat = pad_sequence(x_trans_list, batch_first=True, padding_value=0.0)
    y_trans_mat = pad_sequence(y_trans_list, batch_first=True, padding_value=0.0)

    # 6. 将类别列表转换为张量
    original_indices = torch.tensor(idx_list, dtype=torch.long)
    categories = torch.tensor(cat_list, dtype=torch.long)

    return  x_seq, x_trans_mat, y_trans_mat, lengths, original_indices,categories
