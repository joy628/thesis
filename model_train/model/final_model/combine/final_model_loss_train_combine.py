...
def train_patient_outcome_model(model, 
            ...
            gamma=100, beta=10,kappa=1,  eta=1,theta=1,
            patience: int = 20 ):
    ...
    for ep in range(epochs):
        ...
        for _,flat_data,ts_data, graph_data ,risk, ts_lengths,_, mortality, original_indices in train_loader:
            ...
            # loss_mortality_elementwise = F.binary_cross_entropy(  mortality_prob_pred_last,  y_mortality_true, reduction='none')  # (B)

            # loss_mortality = (loss_mortality_elementwise * mask_seq_mortality).sum() / mask_seq_mortality.sum().clamp(min=1)

            total_loss =  gamma * loss_cah + beta * loss_s_som + kappa * loss_smooth + eta * loss_mortality + theta * loss_risk
            
            train_risk_mortality = loss_risk + loss_mortality
            ...
            current_epoch_losses['train_risk_mortality'] += train_risk_mortality.item()
        ...
        with torch.no_grad():
            for _,flat_data,ts_data, graph_data ,risk, ts_lengths,categories, mortality, original_indices in val_loader:
                ...
                total_epoch_loss_val += (loss_risk_val + loss_mortality_val).item()
...
def test_patient_outcome_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for _, flat_data, ts_data, graph_data, risk, ts_lengths, categories, mortality, _ in test_loader:
            ...
            risk_scores_pred = output["risk_scores"]  # (B, T)
            # Flatten有用部分
            mask_flat = mask_seq.view(-1).bool()
            risk_preds_flat = risk_scores_pred.view(-1)[mask_flat].cpu().numpy()
            risk_trues_flat = y_risk_true.view(-1)[mask_flat].cpu().numpy()
            all_risk_preds.extend(risk_preds_flat)
            all_risk_trues.extend(risk_trues_flat)
            ...
            # 3. per-patient loss分析
            loss_risk_elementwise = F.mse_loss(risk_scores_pred, y_risk_true, reduction='none')
            per_risk = (loss_risk_elementwise * mask_seq).sum(dim=1) / mask_seq.sum(dim=1).clamp(min=1)
            per_patient_risk_losses += per_risk.cpu().tolist()
            # 4. per-patient categories    
            per_patient_cats += categories.cpu().tolist()
    ...
    print(f"Test RMSE for Risk: {rmse:.4f}")
    print(f"Test Mortality - AUROC: {auroc:.4f}, AUPRC: {auprc:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}")

    # 按类别统计loss
    hist_cat = {}
    losses_t = torch.tensor(per_patient_risk_losses) + torch.tensor(per_patient_mort_losses)
    cats_t = torch.tensor(per_patient_cats)
    for i in range(4):
        sel = cats_t == i
        if sel.any():
