...
class MortalityPredictor(nn.Module):
    def __init__(self, ts_dim, lstm_hidden=512,lstm_layers=2, dropout=0.2):
        super().__init__()
        self.ts_dim = ts_dim
    ...
    def forward(self, ts,lengths):
        ...
        return torch.sigmoid(ts_out).squeeze(-1)  # [B, T], mortality probability for each timestep

       
    
# === Full Model ===
class PatientOutcomeModel(nn.Module):
    def __init__(self,  hidden_dim,som=None, pretrained_encoder=None):
        super().__init__()
    ...
    def forward(self, ts_data, length=None):
        ...
        mortality_prob = self.mortality_predictor(ts_emb, length)  # [B, T]
