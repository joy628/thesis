...
class PatientOutcomeModel(nn.Module):
    ...
    def forward(self, flat_data, graph_data, ts_data,length=None):
        ...
        aux_info = self.som_layer(proj_exp)  
        
        return {
                "risk_scores": risk_scores,
                "combine_emb": combine_exp,
    ...
    def compute_loss_smoothness(self, z_e_sample_seq, bmu_indices_flat, alpha_som_q, mask_seq): #
        ...
        """
        z_e_sample_seq: (B, T_max, D_latent), is combine_emb = [static_emb | ts_emb]
        
        """
        B, T, combined_dim = z_e_sample_seq.shape
        
        latent_dim = self.som_layer.som_module.embeddings.size(1)
        z_latent_seq = z_e_sample_seq[:, :, -latent_dim:]  # (B, T, latent_dim)
        ...
        z_e_sample_flat = z_latent_seq.reshape(B * T, latent_dim)            # (B*T, latent_dim)
        
        diff_sq = (z_e_sample_flat - e_prev_bmu_flat.detach()).pow(2).sum(dim=1)  # (B*T,)
